...
라벨 OCR 검사)을 하나의 PySide6 앱으로 통합·고도화한 프로그램입니다.
...
| **라벨 검사** | PDF/웹캠 이미지를 AWS Textract OCR + 바코드(DataMatrix 포함) 검사. **PDF 로드 시 전 페이지 백그라운드 선행 OCR** — 페이지 이동 시 딜레이 없이 결과 표시. LOT 자동 매칭, 규격 자동 선택, GS1 교차 검증, 합불 판정과 주석 이미지 저장 |
