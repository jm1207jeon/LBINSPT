...
| 필드 모아보기 | 뷰어 오른쪽 2/5(기본, 라벨 3/5) 패널에 필드별 검출 객체를 잘라 표로 모아 표시(열=항목, 행=규격 기대 개수). 표는 패널 폭에 맞춰 자동 확대/축소(좌우 스크롤 없음). 빨강 칸=미검출, 주황 테두리=저신뢰, 노랑=기대 초과, 칸 클릭=라벨 위치로 이동. 분할선으로 비율 조절(저장 `overlay.gallery_percent`), 토글로 숨김 |
| 바코드 인식 모드 | 툴바 [▣ 바코드 인식] → 커서 십자 → 라벨 위 바코드(DataMatrix·GS1-128 등) 클릭 → 그 심볼만 판독해 종류·등급·판독값(GS 구분자 표시)·GS1 요소·목록 대조를 창으로 표시, [값 복사]. 전수 탐지가 놓친 심볼도 클릭 지점 주변만 다시 판독. Esc로 해제 |
| 자동 학습 | **기본 모두 꺼짐** — 설정 › 검출 필드·영역 › 자동 학습에서 모듈별 개별 on/off: OCR 패턴 라이브러리 축적 / 라벨 유형 학습·이상 알람 / 동일값 기준 배치 갱신 / 기준정보 DB 자동 축적 (`learning.*`, 구버전 `type_learning.enabled`는 자동 이전). 교정·병합·영역·양식 학습은 사용자가 직접 등록하는 것이라 그대로 |
...
└── tests/                 # xunit 310건 (합성 렌더링 OCR 왕복·DataMatrix 강건성·소스 규약·테마 정합 포함)
